...
export async function GET(req: Request, { params }: { params: Promise<RouteParams> }) {
  ...
  return getInvoiceController(req, routeParams, { requireAuthApi, db, fail, ok, logActivity });
}

export async function PATCH(req: Request, { params }: { params: Promise<RouteParams> }) {
  const routeParams = await params;
  return patchInvoiceController(req, routeParams, { requireAuthApi, db, fail, ok, logActivity });
}
